...
import com.daml.ledger.on.sql.ExtraConfig
...
object MongoLedgerFactory extends LedgerFactory[ReadWriteService, ExtraConfig] {
  ...
  override def manipulateConfig(config: Config[ExtraConfig]): Config[ExtraConfig] =
    config.copy(participants =
      config.participants.map(participantConfig =>
        participantConfig.copy(indexerConfig = participantConfig.indexerConfig.copy(allowExistingSchema = true))
      )
    )
