...
import co.topl.ledger.ParticipantStateIntegrationSpecBase.{
  alice,
  anArchive,
  archives,
  matchPackageUpload,
...
import com.daml.ledger.configuration.{Configuration, LedgerId, LedgerInitialConditions, LedgerTimeModel}
...
abstract class ParticipantStateIntegrationSpecBase(implementationName: String)(implicit
  ...
  implicit private val telemetryContext: TelemetryContext = NoOpTelemetryContext
  ...
  implementationName should {
    ...
    "allocateParty" should {
      ...
      "reject a duplicate submission" in participantState.use { ps =>
        ...
        for {
          ...
          offset2 should be(offsetBuilder.of(3))
          update2.recordTime should be >= rt
          inside(update2) { case PartyAddedToParticipant(_, displayName, _, _, Some(submissionId)) =>
    ...
    "submitTransaction" should {
      "provide an update after a transaction submission" in participantState.use { ps =>
        ...
        } yield offset2 should be(offsetBuilder.of(2))
      }
      ...
      "reject duplicate commands" in participantState.use { ps =>
        ...
        for {
          ...
          offset4 should be(offsetBuilder.of(4))
      ...
      "correctly implement transaction submission authorization" in participantState.use { ps =>
        ...
        for {
          ...
          newParty <- ps
            .stateUpdates(beginAfter = Some(offsetBuilder.of(2)))
            .idleTimeout(IdleTimeout)
            .runWith(Sink.head)
    ...
    "process many party allocations" in participantState.use { ps =>
      ...
      val expectedOffsets = partyIds.map(i => offsetBuilder.of(i)).to(SortedSet)
  ...
  private def submitterInfo(party: Ref.Party, commandId: String = "X") =
    SubmitterInfo(
      ...
      submissionId = None,
      ledgerConfiguration = Configuration(1, LedgerTimeModel.reasonableDefault, Duration.ofSeconds(1))
    )
